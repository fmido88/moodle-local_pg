...
class helper {
     ...
     * @param bool $force Set true to force update.
     * @return void
     */
    public static function write_htaccess($force = false) {

        $version = get_config('local_pg', 'version');
        $updated = get_config('local_pg', 'htaccess_updated');
        if (!$force && $updated && $version <= $updated) {
            return;
        }
        // Get the relative web path to your plugin directory.
        $serveurl = (new moodle_url('/local/pg/index.php'))->get_path();

        $htaccesspath = __DIR__ . '/../.htaccess';

        $htaccesscontent = <<<HTACCESS
        # This file was generated by the Moodle PG plugin.
        # It is recommended to not edit this file manually.
        # Version {$version}
        ...
        <IfModule !mod_rewrite.c>
            ErrorDocument 404 {$serveurl}
        </IfModule>
        HTACCESS;

        // Write to .htaccess file.
        file_put_contents($htaccesspath, $htaccesscontent);
        set_config('htaccess_updated', $version, 'local_pg');
    }
}
